...
<head>
    ...
    <title>@yield('title', 'Dashboard Organizer') - AmikomEventHub</title>
...
<body class="bg-slate-50 text-slate-900 flex min-h-screen">
    ...
    <aside class="w-64 bg-indigo-900 text-indigo-100 flex flex-col p-6 space-y-8 sticky top-0 h-screen">
        ...
        <nav class="flex-1 space-y-2">
            <p class="text-[10px] font-bold uppercase tracking-widest text-indigo-400 mb-4 px-2">Menu Organizer</p>

            <a href="{{ route('organizer.dashboard') }}" class="flex items-center
                gap-3 px-4 py-3 {{ request()->routeIs('organizer.dashboard') ? 'bg-indigo-800
                text-white' : 'hover:bg-indigo-800' }} rounded-xl font-bold transition">
                <svg class="w-5 h-5 {{ request()->routeIs('organizer.dashboard') ?
            ...
            <a href="{{ route('organizer.events.index') }}" class="flex items-center
                gap-3 px-4 py-3 {{ request()->routeIs('organizer.events.*') ? 'bg-indigo-800
                text-white' : 'hover:bg-indigo-800' }} rounded-xl font-bold transition">
                <svg class="w-5 h-5 {{ request()->routeIs('organizer.events.*') ?
                    'text-indigo-300' : 'text-indigo-400' }}" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <main class="flex-1 p-10 overflow-y-auto w-full">
        ...
        <header class="flex justify-between items-center mb-10 w-full col-span-full">
            ...
            <div class="flex items-center gap-4">
                <div class="text-right hidden md:block">
                    <p class="font-bold">{{ auth()->user()->name ?? 'Organizer' }}</p>
                    <p class="text-xs text-slate-400">Penyelenggara Event</p>
                </div>
                <div class="w-12 h-12 bg-white rounded-2xl shadow-sm border flex items-center justify-center p-1">
                    <img
                        src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'Organizer') }}&background=6366f1&color=fff"
                        class="rounded-xl">
                </div>
            </div>
        </header>
        ...
        @if(session('success'))
        <div class="mb-6 px-6 py-4 bg-indigo-50 border border-indigo-200 text-indigo-700 rounded-2xl font-medium flex items-center gap-3">
