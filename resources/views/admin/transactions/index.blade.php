            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50/50 transition {{ $trx->status == 'pending' ? 'text-slate-400' : '' }}">
                    ...
                    <td class="px-8 py-6 text-right">
                        ...
                        <form action="{{ route('admin.transactions.updateStatus', $trx->id) }}" method="POST" onsubmit="return confirm('Tandai transaksi ini sebagai LUNAS?')">
                            @csrf
                            @method('PATCH')
