...
<body class="bg-indigo-900 text-white min-h-screen flex items-center justify-center p-6">
    <div class="max-w-md w-full bg-white text-slate-900 rounded-[2rem] p-8 shadow-2xl my-10">
        <div class="text-center mb-8">
            ...
            <p class="text-slate-500">Pesan tiket event favoritmu di AmikomEventHub</p>
        </div>

        @if(session('success'))
            <div class="bg-green-100 text-green-700 p-4 rounded-xl mb-6 font-bold text-sm text-center">
                {{ session('success') }}
            </div>
        @endif
        ...
        <form action="{{ route('login.post') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium" required autofocus>
            </div>
            <div>
            ...
            <div class="flex items-center gap-2">
                ...
                <label for="remember" class="text-sm text-slate-500 font-medium">Ingat saya</label>
            </div>
        ...
        {{-- Login instan via Google (Socialite SSO) --}}
        ...
        <p class="text-center text-sm text-slate-500 mt-6">
            Belum punya akun? <a href="{{ route('register') }}" class="text-indigo-600 font-bold">Daftar di sini</a>
        </p>
        <p class="text-center text-sm text-slate-500 mt-2">
            Ingin jadi penyelenggara? <a href="{{ route('organizer.register') }}" class="text-indigo-600 font-bold">Daftar sebagai Organizer</a>
        </p>
    </div>
</body>
</html>