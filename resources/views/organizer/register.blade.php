...
<body class="bg-indigo-900 text-white min-h-screen flex items-center justify-center p-6">
    <div class="max-w-md w-full bg-white text-slate-900 rounded-[2rem] p-8 shadow-2xl my-10">
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-black">Daftar Sebagai Organizer</h1>
            <p class="text-slate-500">Selenggarakan event Anda di AmikomEventHub</p>
        </div>
        ...
        <form action="{{ route('organizer.register.post') }}" method="POST" class="space-y-5">
            ...
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">Nama Penyelenggara / Organisasi</label>
                <input type="text" name="organizer_name" value="{{ old('organizer_name') }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium" required>
            </div>
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">Deskripsi (opsional)</label>
                <textarea name="description" rows="3" class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium">{{ old('description') }}</textarea>
            </div>
        ...
        <p class="text-center text-sm text-slate-500 mt-6">
            Sudah punya akun organizer? <a href="{{ route('organizer.login') }}" class="text-indigo-600 font-bold">Masuk di sini</a>
        </p>
    </div>
</body>
</html>